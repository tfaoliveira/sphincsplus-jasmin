...
#define LU8U32(p, i)                                                                             \
    ((((uint32_t)p[i + 3]) << 24) | (((uint32_t)p[i + 2]) << 16) | (((uint32_t)p[i + 1]) << 8) | \
     (((uint32_t)p[i]) << 0))
...
static int crypto_rng(uint8_t *r,      /* random output */
                      uint8_t *n,      /* new key */
...
static void randombytes_internal(uint8_t *x, uint64_t xlen, uint8_t *g, uint8_t *r, uint64_t *pos) {
    while (xlen > 0) {
        if ((*pos) == OUTPUTBYTES) {
            crypto_rng(r, g, g);
            *pos = 0;
...
void randombytes1(uint8_t *x, uint64_t xlen) { randombytes_internal(x, xlen, g1, r1, &pos1); }
