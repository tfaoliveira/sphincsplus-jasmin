...
"""
Script that writes a test for fors_sign 
"""

debug: bool = True

out_path: str = os.path.join("..", "fors_sign_failed_tests.c")

out_str: str = """
void test_fors_sign_failed_tests(void)
{
"""
...
test_files: list[str] = sorted(
    [
        file
        for file in os.listdir("../fors_sign_failed_tests")
        if re.search(r"test_[0-9]+\.txt", file)
    ]
)

with open("test_fors_sign.template", "r") as f:
