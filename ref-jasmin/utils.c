 ...
 */
void treehash(unsigned char *root, unsigned char *auth_path, const spx_ctx *ctx, uint32_t leaf_idx, uint32_t idx_offset,
              uint32_t tree_height,
              void (*gen_leaf)(unsigned char * /* leaf */, const spx_ctx * /* ctx */, uint32_t /* addr_idx */,
...
/* Set the address of the node we're creating. */
#ifdef TEST_ADDRESS
            set_tree_height_jazz(tree_addr, heights[offset - 1] + 1);
            set_tree_index_jazz(tree_addr, tree_idx + (idx_offset >> (heights[offset - 1] + 1)));
#else
            ...
            thash(stack + (offset - 2) * SPX_N, stack + (offset - 2) * SPX_N, 2, ctx, tree_addr);
            offset--;
            ...
            if (((leaf_idx >> heights[offset - 1]) ^ 0x1) == tree_idx) {
                memcpy(auth_path + heights[offset - 1] * SPX_N, stack + (offset - 1) * SPX_N, SPX_N);
            }
        }
    }
