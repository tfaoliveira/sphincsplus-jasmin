...
void fors_gen_sk(unsigned char *sk, const spx_ctx *ctx, uint32_t fors_leaf_addr[8]) {
    prf_addr(sk, ctx, fors_leaf_addr);
}

void fors_gen_skx4(unsigned char *sk0, unsigned char *sk1, unsigned char *sk2, unsigned char *sk3, const spx_ctx *ctx,
...
void fors_sk_to_leafx4(unsigned char *leaf0, unsigned char *leaf1, unsigned char *leaf2, unsigned char *leaf3,
    ...
    thashx4(leaf0, leaf1, leaf2, leaf3, sk0, sk1, sk2, sk3, 1, ctx, fors_leaf_addrx4);
}
...
void fors_gen_leafx4(unsigned char *leaf, const spx_ctx *ctx, uint32_t addr_idx, void *info) {
    ...
    fors_gen_skx4(leaf + 0 * SPX_N, leaf + 1 * SPX_N, leaf + 2 * SPX_N, leaf + 3 * SPX_N, ctx, fors_leaf_addrx4);
...
void fors_sign(unsigned char *sig, unsigned char *pk, const unsigned char *m, const spx_ctx *ctx,
    ...
    for (i = 0; i < SPX_FORS_TREES; i++) {
        ...
        treehashx4(roots + i * SPX_N, sig, ctx, indices[i], idx_offset, SPX_FORS_HEIGHT, fors_gen_leafx4,
