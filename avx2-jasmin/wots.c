 ...
 */
static void gen_chains(unsigned char *out, const unsigned char *in, unsigned int start[SPX_WOTS_LEN],
                       unsigned int steps[SPX_WOTS_LEN], const spx_ctx *ctx, uint32_t addr[8]) {
    ...
    for (i = 0; i < SPX_WOTS_LEN; i += 4) {
        ...
        for (k = 0;; k++) {
            ...
            thashx4(bufs[0], bufs[1], bufs[2], bufs[3], bufs[0], bufs[1], bufs[2], bufs[3], 1, ctx, addrs);
        }
    }
}
 ...
 */
void wots_gen_leafx4(unsigned char *dest, const spx_ctx *ctx, uint32_t leaf_idx, void *v_info) {
    ...
    for (i = 0, buffer = pk_buffer; i < SPX_WOTS_LEN; i++, buffer += SPX_N) {
        ...
        prf_addrx4(buffer + 0 * wots_offset, buffer + 1 * wots_offset, buffer + 2 * wots_offset,
                   buffer + 3 * wots_offset, ctx, leaf_addr);
        ...
        for (k = 0;; k++) {
            ...
            if (k == wots_k) {
                memcpy(info->wots_sig + i * SPX_N, buffer + wots_sign_index * wots_offset, SPX_N);
            }
            ...
            thashx4(buffer + 0 * wots_offset, buffer + 1 * wots_offset, buffer + 2 * wots_offset,
                    buffer + 3 * wots_offset, buffer + 0 * wots_offset, buffer + 1 * wots_offset,
                    buffer + 2 * wots_offset, buffer + 3 * wots_offset, 1, ctx, leaf_addr);
        }
    }

    /* Do the final thash to generate the public keys */
    thashx4(dest + 0 * SPX_N, dest + 1 * SPX_N, dest + 2 * SPX_N, dest + 3 * SPX_N, pk_buffer + 0 * wots_offset,
            pk_buffer + 1 * wots_offset, pk_buffer + 2 * wots_offset, pk_buffer + 3 * wots_offset, SPX_WOTS_LEN, ctx,
            pk_addr);
}
