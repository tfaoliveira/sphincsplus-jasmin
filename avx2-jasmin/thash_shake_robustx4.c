 ...
 */
void thashx4(unsigned char *out0, unsigned char *out1, unsigned char *out2, unsigned char *out3,
             const unsigned char *in0, const unsigned char *in1, const unsigned char *in2, const unsigned char *in3,
             unsigned int inblocks, const spx_ctx *ctx, uint32_t addrx4[4 * 8]) {
    if (inblocks == 1 || inblocks == 2) {
        ...
        for (int i = 0; i < 4; i++) {
            state[SPX_N / 8 + i] = _mm256_set_epi32(
                addrx4[3 * 8 + 1 + 2 * i], addrx4[3 * 8 + 2 * i], addrx4[2 * 8 + 1 + 2 * i], addrx4[2 * 8 + 2 * i],
                addrx4[8 + 1 + 2 * i], addrx4[8 + 2 * i], addrx4[1 + 2 * i], addrx4[2 * i]);
        }
        ...
        state2[(SPX_N / 8) * (1 + inblocks) + 4] =
            _mm256_xor_si256(state2[(SPX_N / 8) * (1 + inblocks) + 4], _mm256_set1_epi64x(0x1f));
        ...
        shake256x4(bitmask0, bitmask1, bitmask2, bitmask3, inblocks * SPX_N, buf0, buf1, buf2, buf3,
